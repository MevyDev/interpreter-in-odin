...
main :: proc() {
	...
	abs_source_path, abs_err := filepath.abs(raw_path, context.allocator)
	defer delete(abs_source_path)
	if abs_err != nil {
		fmt.eprintfln("Error: path %s could not be converted to an absolute path", raw_path)
		return
	}

	source_path, clean_err := filepath.clean(abs_source_path, context.allocator)
	...
	defer delete(source_content_bytes)
	if read_err != nil {
